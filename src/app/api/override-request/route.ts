...
const InputSchema = z.object({
  ...
  requested_event_type: z.enum(["IN", "OUT", "BREAK_START", "BREAK_END"]).default("IN"),
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const { data: row, error } = await sb.from("override_requests").insert({
    tenant_id: session.tenant_id,
    staff_id: session.staff_id,
    requested_location_id: parsed.data.location_id,
    requested_event_type: parsed.data.requested_event_type,
    lat: parsed.data.lat,
    lng: parsed.data.lng,
  ...
  if (error || !row) return NextResponse.json({ ok: false, reason: "INTERNAL", message: error?.message }, { status: 500 });
  ...
  if (managers.length > 0) {
    ...
    void sendEmail({
      ...
      html: `<p><strong>${staff?.full_name ?? "A staff member"}</strong> is requesting an out-of-zone <strong>${parsed.data.requested_event_type}</strong>.</p>
        <ul>
          <li>Location: ${loc?.name ?? "—"}</li>
          <li>Distance from perimeter: ${Math.round(parsed.data.distance_m)} m</li>
          <li>GPS accuracy: &plusmn;${Math.round(parsed.data.accuracy_m)} m</li>
        </ul>
